...
void DetourNavigation::_register_methods() {
	...
	register_method("_on_cache_object_added",
			&DetourNavigation::_on_cache_object_added);
	register_method("_on_cache_object_removed",
			&DetourNavigation::_on_cache_object_removed);
	register_method("_on_static_object_added",
			&DetourNavigation::_on_static_object_added);
	register_method("_on_static_object_removed",
			&DetourNavigation::_on_static_object_removed);
	register_method("_on_tree_exiting", &DetourNavigation::_on_tree_exiting);

	register_property<DetourNavigation, int>("parsed_geometry_type",
			&DetourNavigation::set_parsed_geometry_type,
			&DetourNavigation::get_parsed_geometry_type,
			(int) PARSED_GEOMETRY_STATIC_COLLIDERS,
			GODOT_METHOD_RPC_MODE_DISABLED, GODOT_PROPERTY_USAGE_DEFAULT,
			GODOT_PROPERTY_HINT_ENUM, "Static bodies,Mesh instances");
...
void DetourNavigation::update_tilecache() {
	for (int i = 0; i < get_child_count(); ++i) {
		DetourNavigationMeshCached *navmesh_pc = Object::cast_to<
				DetourNavigationMeshCached>(get_child(i));
		if (navmesh_pc != nullptr) {
			navmesh_pc->get_tile_cache()->update(0.1f,
					navmesh_pc->get_detour_navmesh());
...
void DetourNavigation::add_box_obstacle_to_all(int64_t instance_id,
	...
	for (int i = 0; i < get_child_count(); ++i) {
		DetourNavigationMeshCached *navmesh_pc = Object::cast_to<
				DetourNavigationMeshCached>(get_child(i));
		if (navmesh_pc != nullptr
				&& navmesh_pc->get_dynamic_collision_mask() & collision_layer) {
			unsigned int ref_id = navmesh_pc->add_box_obstacle(position,
					extents, rotationY);
...
void DetourNavigation::add_cylinder_obstacle_to_all(int64_t instance_id,
	...
	for (int i = 0; i < get_child_count(); ++i) {
		...
		if (navmesh_pc != nullptr
				&& navmesh_pc->get_dynamic_collision_mask() & collision_layer) {
			unsigned int ref_id = navmesh_pc->add_cylinder_obstacle(position,
					radius, height);
			navmesh_pc->dynamic_obstacles[instance_id] = (Variant) ref_id;
		}
...
void DetourNavigation::remove_obstacle(CollisionShape *collision_shape) {
	...
	for (int i = 0; i < get_child_count(); ++i) {
		DetourNavigationMeshCached *navmesh_pc = Object::cast_to<
				DetourNavigationMeshCached>(get_child(i));
		if (navmesh_pc != nullptr) {
			navmesh_pc->remove_obstacle(
					navmesh_pc->dynamic_obstacles[collision_shape->get_instance_id()]);
		}
	}
}
			...
			for (int i = 0; i < static_body->get_child_count(); ++i) {
				...
				if (cylinder) {
					add_cylinder_obstacle_to_all(
							...
							cylinder->get_height()
									* transform.get_basis().get_scale().y,
							static_body->get_collision_layer());
				}
			}
...
void DetourNavigation::build_navmesh(DetourNavigationMesh *navmesh) {
	...
	DetourNavigation::collect_mesh_instances(get_children(), meshes, transforms,
	...
	dtnavmesh_gen->init_mesh_data(meshes, transforms, aabbs,
			get_global_transform(), collision_ids);
...
void DetourNavigation::build_navmesh_cached(
	...
	dtnavmesh_gen->init_mesh_data(meshes, transforms, aabbs,
			get_global_transform(), collision_ids);
	dtnavmesh_gen->set_navmesh_parameters(navmesh->navmesh_parameters);
	navmesh->set_generator(dtnavmesh_gen);
	dtnavmesh_gen->build();
...
int DetourNavigation::process_large_mesh(MeshInstance *mesh_instance,
	...
	for (int i = 0; i < array_mesh->get_surface_count(); i++) {
		...
		if (array_mesh->surface_get_primitive_type(i)
				!= Mesh::PRIMITIVE_TRIANGLES)
			return 0;

		if (transformed_aabb.get_size().x < tile_edge_length
				&& transformed_aabb.get_size().z < tile_edge_length) {
			return 0;
		}
		int index_count = 0;

		bool index_format = array_mesh->surface_get_format(i)
		...
		int x_size = (int) std::ceil(
				(base_position.x + make_positive.x
						+ transformed_aabb.get_size().x) / tile_edge_length);
		int z_size = (int) std::ceil(
				(base_position.z + make_positive.z
						+ transformed_aabb.get_size().z) / tile_edge_length);

		std::vector < std::vector<std::list<Vector3>>
				> mesh_instance_array(x_size,
						std::vector < std::list < Vector3 >> (z_size));

		for (int j = 0; j < face_count; j++) {
			Vector3 a = mesh_instance->get_transform().xform(
					mesh_vertices[j * 3 + 0]) + make_positive;
			Vector3 b = mesh_instance->get_transform().xform(
					mesh_vertices[j * 3 + 1]) + make_positive;
			Vector3 c = mesh_instance->get_transform().xform(
					mesh_vertices[j * 3 + 2]) + make_positive;
...
void DetourNavigation::collect_mesh_instances(Array geometries,
		std::vector<Ref<Mesh>> *meshes, std::vector<Transform> *transforms,
		std::vector<AABB> *aabbs, std::vector<int64_t> *collision_ids,
	...
	for (int i = 0; i < geom_size; i++) {
		...
		if (spatial) {
			collect_mesh_instances(spatial->get_children(), meshes, transforms,
					aabbs, collision_ids, navmesh);
		}
	}
